#### updated to calculate sna, co, and cod matrices from the data. Also uses new sna matrix generation that allows for uneven distrubtions of talk off the diag


sim_test = function(n_teams,
                        lines_per_team,
                        team_size,
                        n_codes,
                        soc_type,
  ...
  #### correlation between adj types
  sim_cors = get_cor_adj_set_sim(sim_models[1:3])
  ####
  ...
  ind_est_all = get_ind_est(sim_models$ind_set)
  team_co_mats = ind_est_all$team_probs
  ...
  code_probs_sim = map(.x = team_lines,.f = get_code_probs, splitCol = "team", codes = codes, units = "unique_unit")
  ...
  code_probs_sim = map(flatten(code_probs_sim),data.frame)
  code_probs_sim = map(code_probs_sim,function(x) { x[, codes]})
  # browser()
  ...
  inter_adj_est = pmap(list(code_probs_sim,team_sna_mats,team_co_mats),inter_by_team) #### ERRORING OUT I THINK BECAUSE OF CLASS OF THE MATRICES (SHOULD BE DFS)
  #
  ...
  if(norm){
    ind_est = ind_est_all$normed
  }else{
    ind_est = ind_est_all$prob
  }
  ...
  ind_inter_est_cor = get_ind_inter_cor(ind_est,inter_adj_est)
  ###
  ...
  ind_inter_cor = sim_cors[comparison][[1]]
  ...
  ###add cors to df
  distance_metrics_df$cor = ind_inter_cor
  distance_metrics_df$cor_est = ind_inter_est_cor
  ...
  cor_cor = cor(ind_inter_cor,ind_inter_est_cor)
