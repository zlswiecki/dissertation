...
generate_code_matrix_new = function(n_codes,dom_num = 0,use_real,code_probs = NULL,codes_real = NULL, your_probs = NULL){
  ...
  if(dom_num >0){
    ...
    if(length(your_probs) != n_codes){
      stop("length of provide code probability vector does not match number of codes requested")
    }
